...
const addPatient = async (req, res) => {
  try {
    ...
    res.status(201).json({ data: updatedUser.managePatients });
  } catch (error) {
...
//   const existingPatient = user.managePatients.find(
...
//     return res.status(404).json({ message: "Patient not found" });
//   }
//   // Remove patient manually
//   user.managePatients = user.managePatients.filter(
//     (p) => p._id.toString() !== req.params.id
//   );
//   await user.save();
...
const updateAddresses = async (req, res) => {
  try {
    const { postalcode, state, city, country } = req.body;
    const user = await User.findById(req.user._id);
    if (!user) {
      return res.status(400).json({ message: "User not found" });
    }
    const address = user.manageAddress.id(req.params.id);
    if (!address) {
    ...
    res
      .status(200)
      .json({ data: address, message: "Address updated successfully!" });
  } catch (error) {
    res.status(500).json({ message: error });
  }
};
...
const deleteAddress = async (req, res) => {
  try {
    ...
    await user.save();
    res.status(400).json({ message: "Address removed successfully" });
  } catch (error) {
    res.status(500).json({ message: error });
  }
};
