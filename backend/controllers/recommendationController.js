...
const getPersonalizedRecommendations = asyncHandler(async (req, res) => {
  ...
  const userOrders = await Order.find({ user: userId }).select(
    "orderItems.product"
  );
  ...
  if (purchasedProductIds.length > 0) {
    const purchasedProducts = await MedicineModel.find({
  ...
  if (recommendedProducts.length < 5) {
    const popularProducts = await MedicineModel.find({})
      .sort({ numReviews: -1 })
  ...
  res.send({
    ...
    message: "fetch recomandation successful",
  });
});
...
const getSymptomRecommendations = asyncHandler(async (req, res) => {
  ...
  if (symptomArray.includes("fever"))
    productKeywords.push("pain", "fever");
  if (symptomArray.includes("cough"))
    productKeywords.push("cough", "cold & flu");
  if (symptomArray.includes("headache")) productKeywords.push("pain");
  if (symptomArray.includes("allergies"))
    productKeywords.push("allergy");
