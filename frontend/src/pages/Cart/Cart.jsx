...
import { Clearmycart, GetMyAllcart } from "../../Apicalls/user";
...
const Cart = () => {
  ...
  const getAllCarts = async () => {
    try {
      ...
      const response = await GetMyAllcart();
      if (response.data) {
  ...
  const totals = useMemo(() => {
    const mrp = cartItems.reduce(
      (sum, item) => sum + Number(item.price + 88) * Number(item.qty),
      0
    );
  ...
  return (
    ...
    <ToastContainer position="top-right" autoClose={3000} />
      <Pagewrapper>
        ...
        ) : hasFetched && <Error message="No Cart Found" btntext="Go to Home Page" />}
      </Pagewrapper>
